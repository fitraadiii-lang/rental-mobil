  ...
  <svg width="200" height="60" viewBox="0 0 200 60">
    <defs>
      <linearGradient id="carGrad" x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        <stop offset="100%" style={{stopColor:'#2d4a6f', stopOpacity:1}} />
      </linearGradient>
    </defs>
    <path d="M10 35 Q15 20 35 20 L50 20 L55 12 L85 12 L95 20 L110 20 Q115 20 115 25 L115 35 Q115 40 110 40 L100 40 Q100 48 92 48 Q84 48 84 40 L36 40 Q36 48 28 48 Q20 48 20 40 L15 40 Q10 40 10 35 Z" fill="url(#carGrad)"/>
    <path d="M57 14 L54 19 L70 19 L70 14 Z" fill="#87CEEB" opacity="0.7"/>
    <path d="M72 14 L72 19 L90 19 L85 14 Z" fill="#87CEEB" opacity="0.7"/>
    <text x="5" y="55" fontFamily="Arial Black, sans-serif" fontSize="20" fontWeight="900">
      <tspan fill="#1e3a5f">M</tspan>
      <tspan fill="#f59e0b">OBILLO</tspan>
    </text>
    <text x="115" y="55" fontFamily="Arial" fontSize="6" fill="#666">THE BEST CAR FOR YOU</text>
  </svg>
);
...
const StampAndSignature = () => (
  <div style={{ position: 'relative', width: '180px', height: '100px' }}>
    <div style={{
      ...
      <div style={{ fontSize: '10px', fontWeight: 'bold', color: '#f59e0b', textAlign: 'center' }}>MOBILLO</div>
      <div style={{ fontSize: '6px', color: '#f59e0b', textAlign: 'center' }}>THE BEST CAR</div>
      <div style={{ fontSize: '6px', color: '#f59e0b', textAlign: 'center' }}>FOR YOU</div>
    </div>
    <svg width="100" height="50" style={{ position: 'absolute', right: '0', top: '30px' }}>
      <path d="M10 40 Q20 10 40 30 Q50 40 60 20 Q70 10 80 25 L85 20" 
...
export default function Home() {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      if (carsData && carsData.length > 0) {
        ...
        // Insert default cars jika belum ada
        const defaultCars = [
      ...
      if (servicesData) {
        setServiceHistory(servicesData.map(s => ({
          ...s,
          carId: s.car_id,
          carName: s.car_name,
  ...
  const handleCellClick = (carId, day) => {
    ...
    if (existing) {
      setFormData({ 
        renterName: existing.renterName || existing.renter_name, 
        amount: existing.amount, 
        phone: existing.phone || '',
        address: existing.address || '',
        status: existing.status || 'lunas',
        leaseDate: existing.leaseDate || existing.lease_date || '',
        returnDate: existing.returnDate || existing.return_date || '',
      ...
      setFormData({ 
        renterName: '', 
        amount: car?.price_per_day || car?.pricePerDay || '', 
        phone: '', 
        address: '',
        status: 'lunas',
        leaseDate: dateStr,
  ...
  const saveBooking = async () => {
    if (formData.renterName && formData.amount) {
      ...
      const bookingData = {
        car_id: selectedCell.carId,
        car_name: car?.name,
        car_plate: car?.plate,
        day: selectedCell.day,
        month: currentMonth,
        year: currentYear,
        renter_name: formData.renterName,
      ...
      if (isNew) {
        ...
        if (data) {
          setBookings(prev => ({
            ...prev,
            [selectedCell.key]: { ...data, carId: data.car_id, carName: data.car_name, carPlate: data.car_plate, renterName: data.renter_name, invoiceNo: data.invoice_no, leaseDate: data.lease_date, returnDate: data.return_date }
          }));
        }
      } else {
  ...
  const deleteBooking = async () => {
    const existingId = bookings[selectedCell.key]?.id;
    if (existingId) {
      await supabase.from('bookings').delete().eq('id', existingId);
    }
    setBookings(prev => {
      const newBookings = { ...prev };
      delete newBookings[selectedCell.key];
      return newBookings;
    });
    setShowModal(false);
    setFormData({ renterName: '', amount: '', phone: '', address: '', status: 'lunas', leaseDate: '', returnDate: '', duration: 1 });
  ...
  const saveService = async () => {
    if (serviceData.carId && serviceData.description && serviceData.cost && serviceData.date) {
      ...
      // Save service
      const { data: serviceResult } = await supabase.from('services').insert({
        car_id: Number(serviceData.carId),
        car_name: car?.name,
        car_plate: car?.plate,
      ...
      if (serviceResult) {
        setServiceHistory(prev => [{
          ...serviceResult,
          carId: serviceResult.car_id,
          carName: serviceResult.car_name,
          carPlate: serviceResult.car_plate
        }, ...prev]);
        
        // Also add to expenses
        const { data: expenseResult } = await supabase.from('expenses').insert({
          description: `Servis ${car?.name} (${car?.plate}) - ${serviceData.description}`,
        ...
        if (expenseResult) {
          setExpenses(prev => [expenseResult, ...prev]);
        }
      }
  ...
  const addNewCar = async () => {
    if (newCarData.name && newCarData.plate) {
      const { data } = await supabase.from('cars').insert({
        ...
        price_per_day: Number(newCarData.pricePerDay) || 300000
      }).select().single();
    ...
    if (confirm('Yakin ingin menghapus mobil ini?')) {
      ...
      setBookings(prev => {
        ...
        Object.keys(newBookings).forEach(key => {
          if (newBookings[key].car_id === carId || newBookings[key].carId === carId) {
  ...
  const printInvoice = () => {
    ...
    WinPrint.document.write(`
      <html>
        <head>
          <title>Invoice ${selectedInvoice?.invoiceNo || selectedInvoice?.invoice_no}</title>
          <style>
  ...
  const exportToExcel = () => {
    const monthBookings = Object.values(bookings)
    ...
    csvContent += 'No,No Invoice,Tanggal Sewa,Tanggal Kembali,Mobil,Plat,Penyewa,Alamat,Telepon,Jumlah,Status\n';
    
    monthBookings.forEach((b, idx) => {
      csvContent += `${idx + 1},${b.invoiceNo || b.invoice_no},${formatDateIndo(b.leaseDate || b.lease_date)},${formatDateIndo(b.returnDate || b.return_date)},${b.carName || b.car_name},${b.carPlate || b.car_plate},${b.renterName || b.renter_name},"${b.address || '-'}",${b.phone || '-'},${b.amount},${b.status === 'lunas' ? 'Lunas' : 'Belum Lunas'}\n`;
    });
    ...
    monthExpenses.forEach((e, idx) => {
      csvContent += `${idx + 1},${formatDateIndo(e.date)},${e.category},"${e.description}",${e.amount}\n`;
    });
  ...
  const inputStyle = {
    ...
    color: '#f1f5f9', fontFamily: 'inherit', fontSize: '1rem', boxSizing: 'border-box'
  };
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{
        textAlign: 'center', marginBottom: '30px', padding: '30px',
        background: 'linear-gradient(135deg, rgba(30, 58, 95, 0.5) 0%, rgba(245, 158, 11, 0.2) 100%)',
        borderRadius: '20px', border: '1px solid rgba(245, 158, 11, 0.3)'
      }}>
        <div style={{ marginBottom: '15px' }}><MobilloLogo /></div>
        <h1 style={{ fontSize: '1.5rem', fontWeight: '600', color: '#f1f5f9', margin: '0 0 5px 0' }}>
          {companyInfo.name}
        </h1>
        <p style={{ color: '#94a3b8', margin: 0, fontSize: '0.9rem' }}>{companyInfo.address}</p>
        <button onClick={() => setShowSettingsModal(true)} style={{
          marginTop: '15px', padding: '8px 16px', borderRadius: '8px', border: 'none',
          background: 'rgba(255,255,255,0.1)', color: '#94a3b8', cursor: 'pointer', fontSize: '0.85rem'
        }}>⚙️ Pengaturan</button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '25px', justifyContent: 'center', flexWrap: 'wrap' }}>
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
            ...
            background: activeTab === tab.id ? 'linear-gradient(135deg, #1e3a5f 0%, #f59e0b 100%)' : 'rgba(255,255,255,0.05)',
      ...
      {activeTab === 'calendar' && (
        <div>
          <div style={{
            ...
            marginBottom: '20px', padding: '15px 25px', background: 'rgba(255,255,255,0.05)',
            borderRadius: '15px', flexWrap: 'wrap', gap: '15px'
          }}>
            <button onClick={() => navigateMonth('prev')} style={{
              padding: '10px 20px', borderRadius: '10px', border: 'none',
              background: 'rgba(245, 158, 11, 0.3)', color: '#f59e0b', cursor: 'pointer', fontFamily: 'inherit'
            }}>← Sebelumnya</button>
            <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: '600', color: '#f1f5f9' }}>
          ...
          <div style={{
            marginBottom: '20px', display: 'flex', gap: '25px', justifyContent: 'center',
            flexWrap: 'wrap', padding: '15px', background: 'rgba(255,255,255,0.03)', borderRadius: '12px'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ width: '20px', height: '20px', borderRadius: '6px', background: 'rgba(51, 65, 85, 0.4)' }}></div>
              <span style={{ color: '#94a3b8', fontSize: '0.9rem' }}>Tersedia</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ width: '20px', height: '20px', borderRadius: '6px', background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.5) 0%, rgba(5, 150, 105, 0.5) 100%)' }}></div>
              <span style={{ color: '#94a3b8', fontSize: '0.9rem' }}>Lunas</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ width: '20px', height: '20px', borderRadius: '6px', background: 'linear-gradient(135deg, rgba(239, 68, 68, 0.5) 0%, rgba(220, 38, 38, 0.5) 100%)' }}></div>
              <span style={{ color: '#94a3b8', fontSize: '0.9rem' }}>Belum Lunas</span>
            </div>
          </div>

          <div style={{
            overflowX: 'auto', borderRadius: '20px', background: 'rgba(255,255,255,0.03)',
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '3px', minWidth: '1000px' }}>
              <thead>
                <tr>
                  ...
                  {[...Array(daysInCurrentMonth)].map((_, i) => {
                    ...
                    return (
                      <th key={`day-${i}`} style={{
                        padding: '6px 3px', background: isWeekend ? 'rgba(239, 68, 68, 0.2)' : 'rgba(30, 58, 95, 0.3)',
                        borderRadius: '6px 6px 0 0', fontWeight: '500', fontSize: '0.7rem',
                        color: isWeekend ? '#f87171' : '#94a3b8', minWidth: '40px'
                      }}>{dayNames[dayOfWeek]}</th>
                    );
                  })}
                </tr>
                <tr>
                  <th style={{
                    padding: '12px 10px', background: 'linear-gradient(135deg, #1e3a5f 0%, #2d4a6f 100%)',
                    borderRadius: '10px', fontWeight: '600', fontSize: '0.85rem', minWidth: '140px'
                  }}>Jenis Mobil</th>
                  {[...Array(daysInCurrentMonth)].map((_, i) => {
                    const dayOfWeek = getDayOfWeek(i + 1, currentMonth, currentYear);
                    const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
                    return (
                      <th key={i} style={{
                        padding: '10px 3px', background: isWeekend ? 'rgba(239, 68, 68, 0.3)' : 'rgba(30, 58, 95, 0.5)',
                        borderRadius: '0 0 6px 6px', fontWeight: '600', fontSize: '0.85rem',
                        color: isWeekend ? '#fca5a5' : '#93c5fd'
                      }}>{i + 1}</th>
                    );
                  })}
                </tr>
              ...
              <tbody>
                {cars.map(car => (
                  <tr key={car.id}>
                    <td style={{
                      padding: '10px', background: 'rgba(30, 41, 59, 0.8)', borderRadius: '10px', fontWeight: '500'
                    }}>
                    ...
                    {[...Array(daysInCurrentMonth)].map((_, i) => {
                      ...
                      return (
                        <td key={i} onClick={() => handleCellClick(car.id, day)} style={{
                          padding: '6px 3px',
                          background: isBooked 
                            ? isLunas 
                            ...
                            : 'rgba(51, 65, 85, 0.4)',
                          borderRadius: '6px', cursor: 'pointer', textAlign: 'center', verticalAlign: 'top',
                        ...
                        >
                          {isBooked && (
                            <div style={{ fontSize: '0.6rem' }}>
                              <div style={{ fontWeight: '600', color: isLunas ? '#10b981' : '#ef4444', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '38px' }}>
                                {(booking.renterName || booking.renter_name || '').split(' ')[0]}
                              </div>
                              <div style={{ color: isLunas ? '#6ee7b7' : '#fca5a5', marginTop: '1px' }}>
                                {(Number(booking.amount) / 1000).toFixed(0)}k
                              </div>
                            </div>
                          )}
                        </td>
      ...
      {activeTab === 'stats' && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px', marginBottom: '30px' }}>
            {[
              { label: '💰 Total Sewa', value: currentMonthIncome, color: '#10b981' },
              { label: '✅ Lunas', value: currentMonthLunas, color: '#22c55e' },
              { label: '⏳ Belum Lunas', value: currentMonthBelumLunas, color: '#f97316' },
              { label: '💸 Pengeluaran', value: currentMonthExpenses, color: '#ef4444' },
              { label: '📊 Profit', value: currentMonthLunas - currentMonthExpenses, color: '#3b82f6' },
            ].map((stat, idx) => (
              <div key={idx} style={{
            ...
            <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
              {currentMonthBookings.sort((a, b) => a.day - b.day).map((booking, idx) => (
                <div key={idx} style={{
                  ...
                  background: booking.status === 'lunas' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                  border: `1px solid ${booking.status === 'lunas' ? 'rgba(16, 185, 129, 0.3)' : 'rgba(239, 68, 68, 0.3)'}`,
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px'
                }}>
                  <div>
                    <div style={{ fontWeight: '600', color: '#f1f5f9', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      {booking.renterName || booking.renter_name}
                      <span style={{
                        padding: '2px 8px', borderRadius: '10px', fontSize: '0.6rem',
                        background: booking.status === 'lunas' ? '#166534' : '#991b1b', color: '#fff'
                      }}>{booking.status === 'lunas' ? 'LUNAS' : 'BELUM'}</span>
                    </div>
                    <div style={{ fontSize: '0.75rem', color: '#64748b' }}>
                      {booking.invoiceNo || booking.invoice_no} • {booking.carName || booking.car_name} • Tgl {booking.day}
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ fontWeight: '600', color: booking.status === 'lunas' ? '#10b981' : '#ef4444' }}>{formatCurrency(booking.amount)}</span>
                    <button onClick={() => openInvoice(booking)} style={{
              ...
              {currentMonthBookings.length === 0 && (
                <div style={{ textAlign: 'center', color: '#64748b', padding: '40px' }}>Belum ada booking</div>
              )}
            </div>
          </div>
        </div>
            ...
            <div style={{ maxHeight: '500px', overflowY: 'auto' }}>
              {expenses.map(expense => (
                <div key={expense.id} style={{
                  padding: '12px', borderRadius: '10px', marginBottom: '8px',
                  background: expense.category === 'servis' ? 'rgba(251, 146, 60, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px'
                }}>
                  <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    {expense.category !== 'servis' && (
                      <button onClick={() => deleteExpense(expense.id)} style={{
                        background: 'rgba(239, 68, 68, 0.3)', border: 'none', borderRadius: '6px',
                        padding: '4px 8px', cursor: 'pointer', color: '#f87171', fontSize: '0.75rem'
                      }}>Hapus</button>
                    )}
                  </div>
                </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <button onClick={() => deleteService(service.id)} style={{
                        background: 'rgba(239, 68, 68, 0.3)', border: 'none', borderRadius: '6px',
                        padding: '5px 10px', cursor: 'pointer', color: '#f87171', fontSize: '0.8rem'
                      }}>Hapus</button>
                    </div>
                  </div>
                </div>
      ...
      {activeTab === 'cars' && (
        <div>
          <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'flex-end' }}>
            <button onClick={() => setShowAddCarModal(true)} style={{
              padding: '12px 24px', borderRadius: '12px', border: 'none',
            ...
            {cars.map(car => {
              ...
              return (
                <div key={car.id} style={{
                  padding: '20px', borderRadius: '20px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)'
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
                    ...
                    <button onClick={() => deleteCar(car.id)} style={{
                      background: 'rgba(239, 68, 68, 0.3)', border: 'none', borderRadius: '8px',
                      padding: '8px 12px', cursor: 'pointer', color: '#f87171', height: 'fit-content'
                    }}>🗑️</button>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <div style={{ background: 'rgba(16, 185, 129, 0.1)', padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.7rem', color: '#6ee7b7' }}>Pemasukan</div>
                      <div style={{ fontWeight: '600', color: '#10b981', fontSize: '0.9rem' }}>{formatCurrency(carIncome)}</div>
                    </div>
                    <div style={{ background: 'rgba(239, 68, 68, 0.1)', padding: '12px', borderRadius: '10px' }}>
                      <div style={{ fontSize: '0.7rem', color: '#fca5a5' }}>Servis</div>
                      <div style={{ fontWeight: '600', color: '#ef4444', fontSize: '0.9rem' }}>{formatCurrency(carServiceCost)}</div>
                    </div>
                  </div>
                </div>
              );
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', borderRadius: '20px', padding: '25px', width: '100%', maxWidth: '400px', border: '1px solid rgba(255,255,255,0.1)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Nama Penyewa *</label>
              <input type="text" value={formData.renterName} onChange={(e) => setFormData({ ...formData, renterName: e.target.value })} placeholder="Mr./Mrs. Nama" style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Alamat</label>
              <input type="text" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} placeholder="Alamat" style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>No. Telepon</label>
              <input type="tel" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} placeholder="08xx" style={inputStyle} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
              <div>
                <label style={labelStyle}>Tgl Sewa</label>
                <input type="date" value={formData.leaseDate} onChange={(e) => setFormData({ ...formData, leaseDate: e.target.value })} style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Tgl Kembali</label>
                <input type="date" value={formData.returnDate} onChange={(e) => setFormData({ ...formData, returnDate: e.target.value })} style={inputStyle} />
              </div>
            </div>
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Jumlah Sewa (Rp) *</label>
              <input type="number" value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value })} placeholder="250000" style={inputStyle} />
            </div>
            <div style={{ marginBottom: '15px' }}>
              <label style={labelStyle}>Status</label>
              <div style={{ display: 'flex', gap: '10px' }}>
                <button type="button" onClick={() => setFormData({ ...formData, status: 'lunas' })} style={{
                  flex: 1, padding: '10px', borderRadius: '10px', border: 'none', cursor: 'pointer',
                  background: formData.status === 'lunas' ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 'rgba(255,255,255,0.05)',
                  color: formData.status === 'lunas' ? '#fff' : '#94a3b8', fontFamily: 'inherit'
                }}>✓ Lunas</button>
                <button type="button" onClick={() => setFormData({ ...formData, status: 'belum' })} style={{
                  flex: 1, padding: '10px', borderRadius: '10px', border: 'none', cursor: 'pointer',
                  background: formData.status === 'belum' ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : 'rgba(255,255,255,0.05)',
                  color: formData.status === 'belum' ? '#fff' : '#94a3b8', fontFamily: 'inherit'
                }}>⏳ Belum</button>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: '#94a3b8', cursor: 'pointer', fontFamily: 'inherit' }}>Batal</button>
              {bookings[selectedCell?.key] && <button onClick={deleteBooking} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', background: '#ef4444', color: '#fff', cursor: 'pointer', fontFamily: 'inherit' }}>Hapus</button>}
              <button onClick={saveBooking} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #1e3a5f 0%, #f59e0b 100%)', color: '#fff', cursor: 'pointer', fontFamily: 'inherit' }}>Simpan</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', borderRadius: '20px', padding: '25px', width: '100%', maxWidth: '350px', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <input type="text" value={expenseData.description} onChange={(e) => setExpenseData({ ...expenseData, description: e.target.value })} placeholder="BBM, Parkir" style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Jumlah (Rp)</label>
              <input type="number" value={expenseData.amount} onChange={(e) => setExpenseData({ ...expenseData, amount: e.target.value })} style={inputStyle} />
            </div>
            <div style={{ marginBottom: '15px' }}>
              <label style={labelStyle}>Tanggal</label>
              <input type="date" value={expenseData.date} onChange={(e) => setExpenseData({ ...expenseData, date: e.target.value })} style={inputStyle} />
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowExpenseModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: '#94a3b8', cursor: 'pointer' }}>Batal</button>
              <button onClick={saveExpense} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', background: '#ef4444', color: '#fff', cursor: 'pointer' }}>Simpan</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', borderRadius: '20px', padding: '25px', width: '100%', maxWidth: '350px', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <select value={serviceData.carId} onChange={(e) => setServiceData({ ...serviceData, carId: e.target.value })} style={inputStyle}>
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <input type="text" value={serviceData.description} onChange={(e) => setServiceData({ ...serviceData, description: e.target.value })} placeholder="Ganti oli" style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Biaya (Rp)</label>
              <input type="number" value={serviceData.cost} onChange={(e) => setServiceData({ ...serviceData, cost: e.target.value })} style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Tanggal</label>
              <input type="date" value={serviceData.date} onChange={(e) => setServiceData({ ...serviceData, date: e.target.value })} style={inputStyle} />
            </div>
            <div style={{ marginBottom: '15px' }}>
              <label style={labelStyle}>Bengkel</label>
              <input type="text" value={serviceData.vendor} onChange={(e) => setServiceData({ ...serviceData, vendor: e.target.value })} style={inputStyle} />
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowServiceModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: '#94a3b8', cursor: 'pointer' }}>Batal</button>
              <button onClick={saveService} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', background: '#f97316', color: '#fff', cursor: 'pointer' }}>Simpan</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAddCarModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', borderRadius: '20px', padding: '25px', width: '100%', maxWidth: '350px', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Nama Mobil</label>
              <input type="text" value={newCarData.name} onChange={(e) => setNewCarData({ ...newCarData, name: e.target.value })} placeholder="Toyota Avanza" style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Plat Nomor</label>
              <input type="text" value={newCarData.plate} onChange={(e) => setNewCarData({ ...newCarData, plate: e.target.value })} placeholder="H 1234 AB" style={inputStyle} />
            </div>
            <div style={{ marginBottom: '15px' }}>
              <label style={labelStyle}>Harga/Hari (Rp)</label>
              <input type="number" value={newCarData.pricePerDay} onChange={(e) => setNewCarData({ ...newCarData, pricePerDay: e.target.value })} placeholder="300000" style={inputStyle} />
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowAddCarModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: '#94a3b8', cursor: 'pointer' }}>Batal</button>
              <button onClick={addNewCar} style={{ flex: 1, padding: '12px', borderRadius: '10px', border: 'none', background: '#10b981', color: '#fff', cursor: 'pointer' }}>Tambah</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)', borderRadius: '20px', padding: '25px', width: '100%', maxWidth: '400px', border: '1px solid rgba(255,255,255,0.1)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <input type="text" value={companyInfo.name} onChange={(e) => setCompanyInfo({ ...companyInfo, name: e.target.value })} style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
              <label style={labelStyle}>Alamat</label>
              <input type="text" value={companyInfo.address} onChange={(e) => setCompanyInfo({ ...companyInfo, address: e.target.value })} style={inputStyle} />
            </div>
            <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ marginBottom: '15px' }}>
              <label style={labelStyle}>Info Rekening</label>
              <input type="text" value={companyInfo.bankAccount} onChange={(e) => setCompanyInfo({ ...companyInfo, bankAccount: e.target.value })} style={inputStyle} />
            </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px', overflowY: 'auto' }}>
          <div style={{ background: '#fff', borderRadius: '15px', width: '100%', maxWidth: '700px', maxHeight: '95vh', overflowY: 'auto' }}>
            <div id="invoice-content" style={{ fontFamily: "'Segoe UI', Arial", color: '#1a1a1a' }}>
              ...
              <div style={{ textAlign: 'center', padding: '15px 30px 10px' }}><MobilloLogo /></div>
              <div style={{ textAlign: 'center', padding: '0 30px 15px' }}>
              ...
              <div style={{ padding: '20px 30px 30px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  <StampAndSignature />
                  <div style={{ marginTop: '8px', fontWeight: '600', color: '#f59e0b' }}>Management</div>
                </div>
              </div>
            </div>
